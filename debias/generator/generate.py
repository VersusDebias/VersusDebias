...
def truncate_prompt(prompt):
    ...
    if len(parts) > 3:
        ...
        truncated = prompt
    return truncated
def all_prompts_processed(prompts, ws, json_template_path):
    for prompt_text in prompts:
        prompt = read_json_file(json_template_path)
        #if use pixart, please us the lines below, or just comment it and use the next 3 lines
        # prompt["113"]["inputs"]["text"] = prompt_text
        # prompt["66"]["inputs"]["filename_prefix"] = prompt_text
        # prompt["155"]["inputs"]["seed"] = random.randint(1000, 6000000000)
        prompt["6"]["inputs"]["text"] = prompt_text
        prompt["9"]["inputs"]["filename_prefix"] = truncate_prompt(prompt_text)
...
def generate_images(prompt_file_path, json_template_path, server_address="127.0.0.1:8197"):
    ws = websocket.WebSocket()
    ws.connect("ws://{}/ws?clientId={}".format(server_address, client_id))

    prompts = load_prompts(prompt_file_path)
    if all_prompts_processed(prompts, ws, json_template_path):
